pipeline {
  agent any
  environment {
    DOCKER_IMAGE = 'library/java-app-iaj:v1' // Image name
    KUBE_CONTEXT = 'iaj-java'  // Kube context if you have multiple clusters
    KUBERNETES_NAMESPACE = 'iaj-java'  // Replace with your namespace
  ...
  stages {
    stage('Checkout') {
      steps {
        ...
        checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'ilyas-github', url: 'git@github.com:ilyasjaelani/java-apps-iaj.git']])
      }
    }
    ...
    stage('Rollout Restart Kubernetes') {
      steps {
        withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'ilyas-k3s', contextName: '', credentialsId: 'ilyas-k3s', namespace: 'default', serverUrl: 'https://172.16.25.129:6443']]) {
          sh '''
            ./kubectl rollout restart deployment/java-app-iaj -n $KUBERNETES_NAMESPACE
          '''
        }
      }
    }
